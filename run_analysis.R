...
#we use descriptive activity names to name the activities in the data set and this is done by 
#reading the labels from the activity_labels.txt file

activity.labels <- read.table('./UCI HAR Dataset/activity_labels.txt', header = FALSE)
activity.labels <- as.character(activity.labels[,2])
data.sub$activity <- activity.labels[data.sub$activity]

#we appropriately label the data set with descriptive variable names by replacing the names 
#in data set with names from activity labels
